 ...
 * precisely the failure mode this tool exists to prevent.
 */
final class TruncatedTreeException extends \RuntimeException
{
    public function __construct(public readonly string $repository)
    {
        parent::__construct(sprintf(
            'the git trees API truncated its response for %s — the blobless-clone fallback lands with the write path',
            $repository,
        ));
    }
}
